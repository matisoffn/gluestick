...
module.exports = async (
  ...
  try {
    ...
    presetPackageJson = JSON.parse(
      fs.readFileSync(
        path.join(
          process.cwd(),
          `node_modules/gluestick-preset-${preset}/package.json`,
        ),
        'utf-8',
      ),
    );
  ...
  if (presetPackageJson.version === version) {
    requiredProjectPackageJson.dependencies =
      presetPackageJson.gsProjectDependencies.dependencies;
    requiredProjectPackageJson.devDependencies =
      presetPackageJson.gsProjectDependencies.devDependencies;
  } else {
    const api = 'http://registry.npmjs.org';
    try {
      const payload = await fetch(
        `${api}/gluestick-preset-${preset}`,
      ).then(response => response.json());
      const { gsProjectDependencies } = payload.versions[version];
      requiredProjectPackageJson.dependencies =
        gsProjectDependencies.dependencies;
      requiredProjectPackageJson.devDependencies =
        gsProjectDependencies.devDependencies;
    } catch (error) {
      throw new Error(
        `Error requesting dependency list from npm registry: ${error.message}`,
      );
    }
  }
  ...
  Object.keys(packageJsonTemplate.dependencies).forEach((dep: string): void => {
    if (
      dep === 'gluestick' &&
      !/\d+\.\d+\.\d+.*/.test(normalizedProjectPackageJson.dependencies[dep])
    ) {
      return;
    }
    if (
      ...
      !isValidVersion(
        normalizedProjectPackageJson.dependencies[dep],
        packageJsonTemplate.dependencies[dep],
      )
    ) {
  ...
  ).forEach((dep: string): void => {
    if (
      !normalizedProjectPackageJson.devDependencies[dep] ||
      !isValidVersion(
        normalizedProjectPackageJson.devDependencies[dep],
        packageJsonTemplate.devDependencies[dep],
      )
    ) {
